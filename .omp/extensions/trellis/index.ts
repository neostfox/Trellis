...
function candidateOmpCliJsPaths(): string[] {
  const candidates: string[] = [];
  ...
  const npmPrefix =
    ...
    stringValue(process.env.NPM_CONFIG_PREFIX);
  if (npmPrefix) {
    candidates.push(join(npmPrefix, ...OMP_CLI_JS_SEGMENTS));
    candidates.push(join(npmPrefix, "lib", ...OMP_CLI_JS_SEGMENTS));
  }

  const appData = stringValue(process.env.APPDATA);
  if (appData) {
    candidates.push(join(appData, "npm", ...OMP_CLI_JS_SEGMENTS));
  }
  ...
  for (const pathEntry of pathValue.split(delimiter)) {
    const entry = pathEntry.trim();
    if (!entry) continue;
    candidates.push(join(entry, ...OMP_CLI_JS_SEGMENTS));
    candidates.push(join(dirname(entry), ...OMP_CLI_JS_SEGMENTS));
    candidates.push(join(dirname(entry), "lib", ...OMP_CLI_JS_SEGMENTS));
  }
...
function resolvePiInvocation(): PiInvocation {
  ...
  // OMP CLI paths (higher priority)
  for (const cliJs of candidateOmpCliJsPaths()) {
    if (isExistingFile(cliJs)) {
      return { command: process.execPath, argsPrefix: [cliJs] };
    }
  }

  // Pi CLI paths (legacy fallback)
  ...
  // Command fallback: prefer omp over pi
  return { command: "omp", argsPrefix: [] };
}
...
function readAgentDefinition(
  ...
  const ompPath = join(projectRoot, ".omp", "agents", `${normalized}.md`);
  const piPath = join(projectRoot, ".pi", "agents", `${normalized}.md`);
  const raw = readText(ompPath) || readText(piPath);
  return {
